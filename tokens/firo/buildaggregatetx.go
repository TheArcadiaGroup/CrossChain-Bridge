...
func (b *Bridge) BuildAggregateTransaction(relayFeePerKb int64, addrs []string, utxos []*electrs.ElectUtxo) (rawTx *txauthor.AuthoredTx, err error) {
	...
	return b.NewUnsignedTransaction(txOuts, firoAmountType(relayFeePerKb), inputSource, changeSource, true)
}
...
func (b *Bridge) VerifyAggregateMsgHash(msgHash []string, args *tokens.BuildTxArgs) error {
	if args == nil || args.Extra == nil || args.Extra.BtcExtra == nil || len(args.Extra.BtcExtra.PreviousOutPoints) == 0 {
		return fmt.Errorf("empty firo extra")
	}
...
func (b *Bridge) getUtxosFromElectUtxos(target firoAmountType, addrs []string, utxos []*electrs.ElectUtxo) (total firoAmountType, inputs []*wireTxInType, inputValues []firoAmountType, scripts [][]byte, err error) {
