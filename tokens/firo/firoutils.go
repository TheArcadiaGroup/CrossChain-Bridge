...
type firoAmountType = firoutil.Amount
...
func isValidValue(value firoAmountType) bool {
	return value > 0 && value <= firoutil.MaxSatoshi
}

func newAmount(value float64) (firoAmountType, error) {
	return firoutil.NewAmount(value)
}
...
func (b *Bridge) GetPayToAddrScript(address string) ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("decode firo address '%v' failed. %w", address, err)
	}
