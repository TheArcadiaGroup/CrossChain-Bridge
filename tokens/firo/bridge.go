...
// NewCrossChainBridge new firo bridge
func NewCrossChainBridge(isSrc bool) *Bridge {
	if !isSrc {
		log.Fatalf("firo::NewCrossChainBridge error %v", tokens.ErrBridgeDestinationNotSupported)
	}
	btc.PairID = PairID
...
func (b *Bridge) VerifyChainConfig() {
	...
	case netCustom:
		return
	default:
		log.Fatal("unsupported firo network", "netID", chainCfg.NetID)
	}
}
...
func (b *Bridge) VerifyTokenConfig(tokenCfg *tokens.TokenConfig) error {
	...
	if strings.EqualFold(tokenCfg.Symbol, "FIRO") && *tokenCfg.Decimals != 8 {
		return fmt.Errorf("invalid decimals for FIRO: want 8 but have %v", *tokenCfg.Decimals)
	}
	return nil
}
