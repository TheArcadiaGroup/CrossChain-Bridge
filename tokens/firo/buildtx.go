...
import (
	...
	"github.com/TheArcadiaGroup/firowallet/wallet/txsizes"
...
func (b *Bridge) BuildRawTransaction(args *tokens.BuildTxArgs) (rawTx interface{}, err error) {
	...
	if extra.RelayFeePerKb != nil {
		relayFeePerKb = firoAmountType(*extra.RelayFeePerKb)
	} else {
		...
		relayFeePerKb = firoAmountType(relayFee)
	}
	...
	inputSource := func(target firoAmountType) (total firoAmountType, inputs []*wireTxInType, inputValues []firoAmountType, scripts [][]byte, err error) {
...
func (b *Bridge) BuildTransaction(from string, receivers []string, amounts []int64, memo string, relayFeePerKb int64) (rawTx interface{}, err error) {
	...
	return b.NewUnsignedTransaction(txOuts, firoAmountType(relayFeePerKb), inputSource, changeSource, false)
}
...
func (b *Bridge) selectUtxos(from string, target firoAmountType) (total firoAmountType, inputs []*wireTxInType, inputValues []firoAmountType, scripts [][]byte, err error) {
	...
	for _, utxo := range utxos {
		value := firoAmountType(*utxo.Value)
...
func (b *Bridge) getUtxos(from string, target firoAmountType, prevOutPoints []*tokens.BtcOutPoint) (total firoAmountType, inputs []*wireTxInType, inputValues []firoAmountType, scripts [][]byte, err error) {
	p2pkhScript, err := b.GetPayToAddrScript(from)
	if err != nil {
		return 0, nil, nil, nil, err
	}
	...
	for _, point := range prevOutPoints {
		...
		value := firoAmountType(*output.Value)
		if value == 0 {
...
func (b *Bridge) NewUnsignedTransaction(outputs []*wireTxOutType, relayFeePerKb firoAmountType, fetchInputs txauthor.InputSource, fetchChange txauthor.ChangeSource, isAggregate bool) (*txauthor.AuthoredTx, error) {
	...
	for {
		...
		if maxRequiredFee < firoAmountType(cfgMinRelayFee) {
			maxRequiredFee = firoAmountType(cfgMinRelayFee)
		}
