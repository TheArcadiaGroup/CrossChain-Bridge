...
// ConvertPARTAddress decode part address and convert to BTC address
// nolint:gocyclo // keep it
func (b *Bridge) ConvertPARTAddress(addr, net string) (address btcutil.Address, err error) {
	...
	if oneIndex > 1 {
		...
		if chaincfg.IsBech32SegwitPrefix(prefix) {
			witnessVer, witnessProg, errf := decodePARTSegWitAddress(addr)
			if errf != nil {
				return nil, errf
			}
...
// nolint:dupl // keep it
func decodePARTSegWitAddress(address string) (version byte, regrouped []byte, err error) {
