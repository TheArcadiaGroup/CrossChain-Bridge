...
func Init(btcExtra *tokens.BtcExtraConfig) {
	...
	if btcExtra == nil {
		log.Fatal("Part bridge must config 'BtcExtra'")
	}
...
func initRelayFee(btcExtra *tokens.BtcExtraConfig) {
	if btcExtra.MinRelayFee > 0 {
		...
		minRelayFee := partAmountType(cfgMinRelayFee)
