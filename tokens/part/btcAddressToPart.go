...
import (
	...
	"github.com/giangnamnabka/partutil/base58"
	"github.com/giangnamnabka/partutil/bech32"
)

// ConvertBTCAddress decode btc address and convert to PART address
// nolint:gocyclo // keep it
func (b *Bridge) ConvertBTCAddress(addr, btcNet string) (address partutil.Address, err error) {
	...
	if oneIndex > 1 {
		...
		if bchaincfg.IsBech32SegwitPrefix(prefix) {
			...
			case 20:
				return partutil.NewAddressWitnessPubKeyHash(witnessProg, lchainConfig)
			case 32:
				return partutil.NewAddressWitnessScriptHash(witnessProg, lchainConfig)
			default:
	...
	if len(addr) == 130 || len(addr) == 66 {
		...
		return partutil.NewAddressPubKey(serializedPubKey, lchainConfig)
	}
	...
	case 20: // P2PKH or P2SH
		...
		case isP2PKH:
			return partutil.NewAddressPubKeyHash(hash160, lchainConfig)
		case isP2SH:
			return partutil.NewAddressScriptHashFromHash(hash160, lchainConfig)
		default:
