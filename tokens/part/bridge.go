...
// NewCrossChainBridge new part bridge
func NewCrossChainBridge(isSrc bool) *Bridge {
	if !isSrc {
		log.Fatalf("part::NewCrossChainBridge error %v", tokens.ErrBridgeDestinationNotSupported)
	}
	btc.PairID = PairID
...
func (b *Bridge) VerifyTokenConfig(tokenCfg *tokens.TokenConfig) error {
	...
	if strings.EqualFold(tokenCfg.Symbol, "PART") && *tokenCfg.Decimals != 8 {
		return fmt.Errorf("invalid decimals for PART: want 8 but have %v", *tokenCfg.Decimals)
	}
	return nil
}
