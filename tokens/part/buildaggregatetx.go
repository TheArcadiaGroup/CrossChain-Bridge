...
func (b *Bridge) BuildAggregateTransaction(relayFeePerKb int64, addrs []string, utxos []*electrs.ElectUtxo) (rawTx *txauthor.AuthoredTx, err error) {
	...
	return b.NewUnsignedTransaction(txOuts, partAmountType(relayFeePerKb), inputSource, changeSource, true)
}
...
func (b *Bridge) VerifyAggregateMsgHash(msgHash []string, args *tokens.BuildTxArgs) error {
	if args == nil || args.Extra == nil || args.Extra.BtcExtra == nil || len(args.Extra.BtcExtra.PreviousOutPoints) == 0 {
		return fmt.Errorf("empty part extra")
	}
...
func (b *Bridge) getUtxosFromElectUtxos(target partAmountType, addrs []string, utxos []*electrs.ElectUtxo) (total partAmountType, inputs []*wireTxInType, inputValues []partAmountType, scripts [][]byte, err error) {
