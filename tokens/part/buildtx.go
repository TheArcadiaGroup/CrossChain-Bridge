...
import (
	...
	"github.com/giangnamnabka/partwallet/wallet/txauthor"
	"github.com/giangnamnabka/partwallet/wallet/txrules"
	"github.com/giangnamnabka/partwallet/wallet/txsizes"
)
...
func (b *Bridge) BuildRawTransaction(args *tokens.BuildTxArgs) (rawTx interface{}, err error) {
	...
	if extra.RelayFeePerKb != nil {
		relayFeePerKb = partAmountType(*extra.RelayFeePerKb)
	} else {
		...
		relayFeePerKb = partAmountType(relayFee)
	}
	...
	inputSource := func(target partAmountType) (total partAmountType, inputs []*wireTxInType, inputValues []partAmountType, scripts [][]byte, err error) {
...
func (b *Bridge) BuildTransaction(from string, receivers []string, amounts []int64, memo string, relayFeePerKb int64) (rawTx interface{}, err error) {
	...
	return b.NewUnsignedTransaction(txOuts, partAmountType(relayFeePerKb), inputSource, changeSource, false)
}
...
func (b *Bridge) selectUtxos(from string, target partAmountType) (total partAmountType, inputs []*wireTxInType, inputValues []partAmountType, scripts [][]byte, err error) {
	...
	for _, utxo := range utxos {
		value := partAmountType(*utxo.Value)
...
func (b *Bridge) getUtxos(from string, target partAmountType, prevOutPoints []*tokens.BtcOutPoint) (total partAmountType, inputs []*wireTxInType, inputValues []partAmountType, scripts [][]byte, err error) {
	p2pkhScript, err := b.GetPayToAddrScript(from)
	if err != nil {
		return 0, nil, nil, nil, err
	}
	...
	for _, point := range prevOutPoints {
		...
		value := partAmountType(*output.Value)
		if value == 0 {
...
// NewUnsignedTransaction ref partwallet
// ref. https://github.com/partsuite/partwallet/blob/b07494fc2d662fdda2b8a9db2a3eacde3e1ef347/wallet/txauthor/author.go
...
func (b *Bridge) NewUnsignedTransaction(outputs []*wireTxOutType, relayFeePerKb partAmountType, fetchInputs txauthor.InputSource, fetchChange txauthor.ChangeSource, isAggregate bool) (*txauthor.AuthoredTx, error) {
	...
	for {
		...
		if maxRequiredFee < partAmountType(cfgMinRelayFee) {
			maxRequiredFee = partAmountType(cfgMinRelayFee)
		}
