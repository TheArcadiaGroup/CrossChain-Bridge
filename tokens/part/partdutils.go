...
import (
	...
	"github.com/giangnamnabka/partd/btcec"
	"github.com/giangnamnabka/partd/chaincfg"
	"github.com/giangnamnabka/partd/chaincfg/chainhash"
	"github.com/giangnamnabka/partd/txscript"
	"github.com/giangnamnabka/partd/wire"
	"github.com/giangnamnabka/partutil"
)

type partAmountType = partutil.Amount
...
func isValidValue(value partAmountType) bool {
	return value > 0 && value <= partutil.MaxSatoshi
}

func newAmount(value float64) (partAmountType, error) {
	return partutil.NewAmount(value)
}
...
func (b *Bridge) GetPayToAddrScript(address string) ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("decode part address '%v' failed. %w", address, err)
	}
